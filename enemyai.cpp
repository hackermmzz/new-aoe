...
static int point[10000][2];
static int Path[10000][2];
...
static int Pathfind(){
    ...
    for(int i=0;i<127;i++){
        for(int j=0;j<127;j++){
            if((enemyInfo.theMap[i][j]+enemyInfo.theMap[i][j+1]>=9)&&enemyInfo.theMap[i][j]!=enemyInfo.theMap[i][j+1]){
                point[z][0]=i;
                point[z][1]=j;
                z++;
            }
            else if((enemyInfo.theMap[i][j]+enemyInfo.theMap[i+1][j]>=9)&&enemyInfo.theMap[i][j]!=enemyInfo.theMap[i+1][j]){
                point[z][0]=i;
                point[z][1]=j;
                z++;
            }
    ...
    if(enemyInfo.buildings[0].BlockDR<64&&enemyInfo.buildings[0].BlockUR<64){
        for(int i=0;i<z;i++){
            if(point[i][1]<64){
                Path[n][0]=point[i][0];
                Path[n][1]=point[i][1];
...
//            qDebug()<<Blood[i];
    ...
    for(int i=0;i<enemyInfo.armies.size();i++){
        if(enemyInfo.armies[i].Sort==3){
            Dis[i]=calDistance(enemyInfo.armies[i].DR,enemyInfo.armies[i].UR,Path[1][0]*33,Path[1][1]*33);
            if (Dis[i]<mindis) {mindis=Dis[i];
            Aroundsum=i;
           }
            }
    }
    armystate[Aroundsum]=AROUND;
    HumanMove(enemyInfo.armies[Aroundsum].SN,78*35,22*35);}
//    if(mode>=1&&mode<4&&armystate[Aroundsum]==AROUND&&AroundLock[Aroundsum]==0){
//        timer[Aroundsum]=g_frame;
////        if(a%2==0)
//        if(Path[Pathn][0]!=LastDR&&Path[Pathn][1]!=LastUR)
//        HumanMove(enemyInfo.armies[Aroundsum].SN,Path[Pathn][0]*35,Path[Pathn][1]*35);
//        LastDR=Path[Pathn][0];
//        LastUR=Path[Pathn][1];
////        else HumanMove(enemyInfo.armies[i].SN,Path[Pathn][0]*33,Path[Pathn][1]*33);
//        AroundLock[Aroundsum]=1;
//        qDebug()<<Pathn<<Path[Pathn][0]<<Path[Pathn][1]<<g_frame;
//    }
//    else if(mode>=1&&mode<4&&armystate[Aroundsum]==AROUND&&AroundLock[Aroundsum]==1){
//        if(g_frame-timer[Aroundsum]>250){
//            AroundLock[Aroundsum]=0;
//             qDebug()<<g_frame;
//        }
//    }
//    if(Pathn<Pathnum-1)
//    Pathn=(g_frame-15)/30+1;
//    qDebug()<<armystate[Aroundsum]<<AroundLock[Aroundsum]<<g_frame;
////    else armystate[Aroundsum]==WAITING;
