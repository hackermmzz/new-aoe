...
/////////////////////////////
void Init(){
    mapInfo=vector<vector<int>> (MAP_L,vector<int>(MAP_U));
     tmp=vector<vector<int>> (MAP_L,vector<int>(MAP_U));
                    ...
                    [&](){
                        ...
                        if(!upgrade1){
                            for(auto&obj:info.buildings){
                                if(obj.Type==BUILDING_STOCK&&obj.Percent>=100){
                                    if(obj.Project==0){
                                        if(info.Meat>=BUILDING_STOCK_UPGRADE_CLOSER_ATTACK_FOOD){
    ...
    //调动所有人来采矿，盖仓库
    {
        ...
        nodes["crazy"]=new Node(
                    ...
                    [&](){
                        ...
                        //只对第一大金矿附近建仓库
                        vector<tagFarmer>farmers=findTargetObj(info.farmers,[&](tagFarmer f){
                                auto&v=enemyLand;
                                cout<<v.front().x<<" "<<v.front().y<<endl;;
                                return f.FarmerSort==FARMERTYPE_FARMER&&blockIndex[f.BlockDR][f.BlockUR]==blockIndex[enemyLand.front().x][enemyLand.front().y];
        });
                        static array<int,2>pos={INT_MAX,INT_MAX};
                        if(pos[0]==INT_MAX){
...
void UsrAI::UpdateAll()
{
    ...
    //更新地图
    {
        ...
        for(int i=0;i<MAP_L;++i)mapInfo[i]=tmp[i];
        //将障碍物标记到地图
    ...
    //更新各区块所属的大陆
    {
        for(auto&v:blockIndex)fill(v.begin(),v.end(),0);
        int idx=0;
        auto&theMap=*(info.theMap);
